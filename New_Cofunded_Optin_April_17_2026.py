...
df_listing.to_csv(f'/home/thotat.vc/analytics_scripts/FOS_SOS_Cofunded_listing_raw.csv')
...
# 2. Fetch only the specific columns from all four tabs
df1 = get_only_offer_id_column("Offer IDs")
df2 = get_only_offer_id_column("50-50 Offers")
#df3 = get_only_offer_id_column("8+5 Offers")   # replace with actual tab name
#df4 = get_only_offer_id_column("10+5 Offers")   # replace with actual tab name
...
df_combined.to_csv('/home/thotat.vc/analytics_scripts/New_April_cofunded_offers.csv', index=False)
...
'''
    ...
    #original_cc = ['thotat.vc@flipkart.com']
    original_message_id = '<CADLy_NGoiRvv6ut0s+T479J53hzUtYy54906HptPiffs+_3aBQ@mail.gmail.com>'
    ...
    msg['Subject'] = f"Opt-In Status Update: Co-Funded FOS & SOS Levels"
    ...
    In the drive file name mentioned as (Cofunded_Optin_Report_SOS_FOS_)<br><br>
    <b>Regards,<br>
    Vamsi</b>
    """
    ...
    df_1_edited.to_csv(f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}.csv", index=False)

    df_1_edited[df_1_edited['owner']=='KAM'].to_csv(f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}_KAM.csv")
    df_1_edited[df_1_edited['owner']=='RE'].to_csv(f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}_RE.csv")
    df_1_edited[df_1_edited['owner']=='UM'].to_csv(f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}_UM.csv")


        # with pd.ExcelWriter(f"/home/thotat.vc/Downloads/Cofunded_Report_{today}.xlsx") as writer:
    ...
    filename =f"Cofunded_Optin_Report_SOS_FOS_{today}.csv"
    file_path =f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}.csv" 
    file_path_kam=f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}_KAM.csv"
    file_path_re=f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}_RE.csv"
    file_path_um=f"/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}_UM.csv"
...
import pandas as pd
import datetime
# today = datetime.datetime.now().strftime("%d_%b_%Y") 


# Define 'today' as yesterday's date
today = datetime.date.today() - datetime.timedelta(days=0)

# Format it as dd_Mon_YYYY
today = today.strftime("%d_%b_%Y")

print(today)
#Read the base file (20th April)
df_base_optin = pd.read_csv(r'/home/thotat.vc/analytics_scripts/Main_base_file_21st_april_optin_90%.csv')

# Read 19th April opt-in file
df_today_optin = pd.read_csv(fr'/home/thotat.vc/Downloads/Cofunded_Optin_Report_SOS_FOS_{today}.csv')

# Rename the column properly with fixed date (19-Apr-2026)
df_today_optin = df_today_optin.rename(columns={f'Opted-in listing_id': f'LID Opt-in {today}'})

# Select relevant columns
cols = ['listing_id', f'LID Opt-in {today}']
df_today_optin_1 = df_today_optin[cols]

# Strip spaces from listing_id for consistency
...
# Merge base with today's opt-in
df_base_optin_optout_1 = df_base_optin.merge(df_today_optin_1, on='listing_id', how='left')

# df_base_optin_optout_1



# Replace "Not opted-in" and missing values with "Opt-out"
df_base_optin_optout_1[f'LID Opt-in {today}'] = (df_base_optin_optout_1[f'LID Opt-in {today}'].fillna('Not opted-in'))


# df_base_optin_optout_1.to_csv(r'C:\Users\thotat.vc\Downloads\19th_and_21st_april_optin_file.csv', index=False)

# Create a DataFrame with only Opt-out rows
df_base_optin_1 = df_base_optin_optout_1[df_base_optin_optout_1[f'LID Opt-in {today}'] == 'Not opted-in']

df_base_optin_1

# Save only Opt-out rows
df_base_optin_1.to_csv(fr'/home/thotat.vc/Downloads/{today}_optout_only_Overaall.csv', index=False)

# Save full output

import ast
today = datetime.datetime.now().strftime("%d_%b_%Y") 
# 1. FIX THE "CHARACTER SOUP": Convert strings back to real lists
# (Only needed if your data was loaded from a CSV/Excel where lists became text)
df_base_optin_1['offer_ids'] = df_base_optin_1['offer_ids'].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)

# 2. PRE-CALCULATE: Create simple columns for FOS and SOS orders
# This removes the need for complex lambdas inside the groupby
df_base_optin_1['FOS_sum'] = df_base_optin_1['orders'].where(df_base_optin_1['SOS / FOS Flag'] == 'FOS', 0)
df_base_optin_1['SOS_sum'] = df_base_optin_1['orders'].where(df_base_optin_1['SOS / FOS Flag'] == 'SOS', 0)

# 3. AGGREGATE: Clean, unique, and straightforward
seller_agg_df = df_base_optin_1.groupby(['seller_id', 'owner', 'KAM'], as_index=False).agg({
    'orders': 'sum',
    'FOS_sum': 'sum',
    'SOS_sum': 'sum',
    'sell_bu': lambda x: list(set(x)), # Unique list of BUs
    'offer_ids': lambda x: list(set([item for sublist in x for item in sublist])) # Unique flattened list of IDs
})

# 4. FORMAT: Calculate percentages
platform_orders = 5244707
cols_to_pct = {'orders': 'Platform_DW%', 'SOS_sum': 'SOS/Platform DW%', 'FOS_sum': 'FOS/Platform DW%'}

for col, name in cols_to_pct.items():
    seller_agg_df[name] = ((seller_agg_df[col] / platform_orders) * 100).round(2).astype(str) + '%'

# Final Cleanup: Rename columns to match your desired output
seller_agg_df.rename(columns={'FOS_sum': 'FOS_orders', 'SOS_sum': 'SOS_orders'}, inplace=True)


seller_agg_df.to_csv(fr'/home/thotat.vc/Downloads/{today}_optout_overall_seller_agg.csv', index=False)


# ... [Your existing code above] ...
seller_agg_df.rename(columns={'FOS_sum': 'FOS_orders', 'SOS_sum': 'SOS_orders'}, inplace=True)

# Save to local CSV (Optional, keep if you still want the local copy)
seller_agg_df.to_csv(fr'/home/thotat.vc/Downloads/{today}_Overall_opt_out_seller_agg_report.csv', index=False)

# =====================================================================================
# ================== GOOGLE SHEETS UPLOAD PART ========================================
# =====================================================================================
import gspread
from gspread_dataframe import set_with_dataframe
...
SERVICE_ACCOUNT_FILE = r'/home/thotat.vc/analytics_scripts/creds.json'

# --- THE FIX: Load the credentials and authorize the gspread client (gc) ---
credentials = google.oauth2.service_account.Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE, scopes=scopes)
gc = gspread.authorize(credentials)

# 2. Open the Existing Google Sheet
SPREADSHEET_ID = '15Tub3VgGnrW4roKQL-WFxZ3-utI5tc_JvXPVslvMq6E' 
spreadsheet = gc.open_by_key(SPREADSHEET_ID)

# 3. Create the New Tab
tab_name = f'{today} vs 21 Apr 2026 Opt-Out  Overall'

try:
    # Try to create a new worksheet (tab)
    worksheet = spreadsheet.add_worksheet(
        title=tab_name, 
        rows=str(len(seller_agg_df) + 100), 
        cols=str(len(seller_agg_df.columns) + 5)
    )
    print(f"Created new tab: '{tab_name}'")
except gspread.exceptions.APIError:
    # If a tab with this name already exists, open it and clear the old data
    print(f"Tab '{tab_name}' already exists. Overwriting data...")
    worksheet = spreadsheet.worksheet(tab_name)
    worksheet.clear()

# 4. Write the DataFrame to the Tab
set_with_dataframe(worksheet, seller_agg_df)

print(f"✅ Successfully pasted data into Google Sheets tab:{today}!")
