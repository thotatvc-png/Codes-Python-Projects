...
#-------------------------CODE--EDITED--HERE for KAM names--------------------


    df_1_1['KAM'] = df_1_1['KAM'].astype(str).str.strip().str.upper()
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ADITI' if str(x).strip().lower() in ['aditi'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'AFFAN NADEEM' if str(x).strip().lower() in ['affan nadeem','affan'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'AISHWARYA' if str(x).strip().lower() in ['aishwarya'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'AKASH PATTANAYAK' if str(x).strip().lower() in ['akash pattanayak'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'AMIT AGARWAL' if str(x).strip().lower() in ['amit agarwal'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ANIMESH JAIN' if str(x).strip().lower() in ['animesh','animesh jain'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ANUKRITI MISHRA' if str(x).strip().lower() in ['anukriti mishra'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ANUSHREE PANT' if str(x).strip().lower() in ['anushree','anushree pant'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ARANYA' if str(x).strip().lower() in ['aranya'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ASHIMA MATHUR' if str(x).strip().lower() in ['ashima mathur'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ASHIS JANA' if str(x).strip().lower() in ['ashis','ashis jana'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ASMITA' if str(x).strip().lower() in ['asmita'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'EKTA' if str(x).strip().lower() in ['ekta'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'GEETASHRI' if str(x).strip().lower() in ['geetashri'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'HARSHIKA' if str(x).strip().lower() in ['harshika'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'HARSHITA PANDEY' if str(x).strip().lower() in ['harshita pandey','harshita'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'KIRTI DANG' if str(x).strip().lower() in ['kirti dang'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'KUNAL THAKUR' if str(x).strip().lower() in ['kunal thakur'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'NAMAN' if str(x).strip().lower() in ['naman'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'PABITRA BOWLARY' if str(x).strip().lower() in ['pabitra','pabitra bowlary'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'POOJA RAMESH' if str(x).strip().lower() in ['pooja ramesh','pooja'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'PRATIK NAG' if str(x).strip().lower() in ['pratik','pratik nag'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'PULKIT' if str(x).strip().lower() in ['pulkit'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'RAJESH' if str(x).strip().lower() in ['rajesh'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'RIYA' if str(x).strip().lower() in ['riya'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'ROHIT ASHOK' if str(x).strip().lower() in ['rohit','rohit ashok'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'RUTIKA GAHERWAR' if str(x).strip().lower() in ['rutika gaherwar'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SAKSHI BAHETI' if str(x).strip().lower() in ['sakshi baheti'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SANSRITI BAJORIA' if str(x).strip().lower() in ['sansriti','sansriti bajoria'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SHIVANI AGARWAL' if str(x).strip().lower() in ['shivani','shivani agarwal'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SHIVENDRA' if str(x).strip().lower() in ['shivendra'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SHOVAN DAS' if str(x).strip().lower() in ['shovan das'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SHRUSHTI PATIL' if str(x).strip().lower() in ['shrushti','shrushti patil'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'SHUBHANKAR' if str(x).strip().lower() in ['shubhankar','sagar bansal'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'UTKARSH UPADHYAY' if str(x).strip().lower() in ['utkarsh','utkarsh upadhyay'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'VANDANA RANA' if str(x).strip().lower() in ['vandana','vandana rana'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'VENU' if str(x).strip().lower() in ['venu'] else x)
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'VISHAL SINGHA' if str(x).strip().lower() in ['vishal','vishal singha','jerin'] else x)    
    df_1_1['KAM'] = df_1_1['KAM'].apply(lambda x: 'NIDHI' if str(x).strip().lower() in ['nidhi','nidhi gupta','Nidhi Gupta'] else x)

#-----------------------TILL---HERE----NEW-------PART---ADDED--------ENDS---HERE------------------------------


#---------------------KAM--LEVEL---CHANGING--NAMES---INPUT LEVEL------------------------------

#------------------------CODE---EDITED-------------------FROM-------------HERE------
# Step 1: Load Input KAM Mapping
    df_universe = pd.read_csv('/home/thotat.vc/analytics_scripts/Input_KAM_Mapping.csv', encoding='cp1252')

# Step 2: Normalize KAM names in df_universe
    df_universe['KAM'] = df_universe['KAM'].astype(str).str.strip().str.lower()

    kam_mapping = {
    'aditi': 'ADITI',
    'affan nadeem': 'AFFAN NADEEM', 'affan': 'AFFAN NADEEM',
    'aishwarya': 'AISHWARYA',
    'akash pattanayak': 'AKASH PATTANAYAK',
    'amit agarwal': 'AMIT AGARWAL',
    'animesh': 'ANIMESH JAIN', 'animesh jain': 'ANIMESH JAIN',
    'anukriti mishra': 'ANUKRITI MISHRA',
    'anushree': 'ANUSHREE PANT', 'anushree pant': 'ANUSHREE PANT',
    'aranya': 'ARANYA',
    'ashima mathur': 'ASHIMA MATHUR',
    'ashis': 'ASHIS JANA', 'ashis jana': 'ASHIS JANA',
    'asmita': 'ASMITA',
    'ekta': 'EKTA',
    'geetashri': 'GEETASHRI',
    'harshika': 'HARSHIKA',
    'harshita': 'HARSHITA PANDEY', 'harshita pandey': 'HARSHITA PANDEY',
    'kirti dang': 'KIRTI DANG',
    'kunal thakur': 'KUNAL THAKUR',
    'naman': 'NAMAN',
    'pabitra': 'PABITRA BOWLARY', 'pabitra bowlary': 'PABITRA BOWLARY',
    'pooja': 'POOJA RAMESH', 'pooja ramesh': 'POOJA RAMESH',
    'pratik': 'PRATIK NAG', 'pratik nag': 'PRATIK NAG',
    'pulkit': 'PULKIT',
    'rajesh': 'RAJESH',
    'riya': 'RIYA',
    'rohit': 'ROHIT ASHOK', 'rohit ashok': 'ROHIT ASHOK',
    'rutika gaherwar': 'RUTIKA GAHERWAR',
    'sakshi baheti': 'SAKSHI BAHETI',
    'sansriti': 'SANSRITI BAJORIA', 'sansriti bajoria': 'SANSRITI BAJORIA',
    'shivani': 'SHIVANI AGARWAL', 'shivani agarwal': 'SHIVANI AGARWAL',
    'shivendra': 'SHIVENDRA',
    'shovan das': 'SHOVAN DAS',
    'shrushti': 'SHRUSHTI PATIL', 'shrushti patil': 'SHRUSHTI PATIL',
    'shubhankar': 'SHUBHANKAR', 'sagar bansal': 'SHUBHANKAR',
    'utkarsh': 'UTKARSH UPADHYAY', 'utkarsh upadhyay': 'UTKARSH UPADHYAY',
    'vandana': 'VANDANA RANA', 'vandana rana': 'VANDANA RANA',
    'venu': 'VENU',
    'vishal': 'VISHAL SINGHA', 'vishal singha': 'VISHAL SINGHA', 'jerin': 'VISHAL SINGHA',
    'nidhi': 'NIDHI', 'nidhi gupta': 'NIDHI','Nidhi Gupta':'NIDHI'
}

    df_universe['KAM'] = df_universe['KAM'].map(kam_mapping).fillna(df_universe['KAM']).str.upper()

    
    excluded_fsns=['LSTYFHHGDHXFGSHFYWBKUYARN','LSTYFHHGDHRKDZSFWSPK7BNED','LSTYFHHGDHUYDHW34EPQ3MXMB','LSTRAFGUEE5VHY2HZSQYYHBLB','LSTCOZGGDC8WM4ZQJDMQFHKEY','LSTMNZGSVRVX6QUGKCQ6EQ2NJ']
    df_universe=df_universe[~df_universe['LID'].isin(excluded_fsns)]


    # Step 3: Mark INPUT vs NON-INPUT listings
    df_1_1['listing_id'] = df_1_1['listing_id'].astype(str).str.strip()
    ...
    df_final_mapping = df_universe.groupby('LID').agg(
        opted_in_offer_id=('Offer ID', list),
        KAM=('KAM', 'first')
    ).reset_index()

    # Step 4: Prepare final mapping from df_universe (no need to reload the same file again)
    df_final_mapping = df_universe.rename(columns={'KAM': 'Input KAM', 'LID': 'listing_id'})
    
    

    df_final_mapping = df_final_mapping.groupby('listing_id').agg({
        'Input KAM': 'first',
        'Offer ID': lambda x: ', '.join(set(x.dropna().astype(str)))
    }).reset_index()

    df_1_1 = df_1_1.rename(columns={'KAM': 'Primary KAM'})

    # Step 5: Merge Input KAM mapping
    df_1_1 = df_1_1.merge(df_final_mapping, on=['listing_id'], how='left')

    # Step 6: Create unified KAM column and enforce uppercase
    df_1_1['KAM'] = df_1_1['Input KAM'].fillna('No KAM')

#----unhashed
    # df_1_1['KAM'] = df_1_1['Input KAM'].fillna(df_1_1['Primary KAM']).astype(str).str.strip().str.upper()

    # Step 7: Merge Lead mapping (now keys will match correctly)
    df_lead_mapping = pd.read_csv('/home/thotat.vc/analytics_scripts/KAM_Lead_Mapping.csv', encoding='cp1252')
    df_lead_mapping['KAM'] = df_lead_mapping['KAM'].astype(str).str.strip().str.upper()

    df_1_1 = df_1_1.merge(df_lead_mapping, on=['KAM'], how='left')
    df_1_1['Lead']=df_1_1['Lead'].fillna('No Lead')
    
#---------------------------------TILL---HERE--------CODE-EDITED-----------------------
    ...
    df_optin_summary = df_optin.groupby('listing_id').agg(opted_in_offer_id=('offer_id', list)).reset_index()
    df_optin_new_fpo = df_optin[df_optin["offer_id"].isin(offer_ids_new_fpo)].copy()


    # >>> START: NEW ELIGIBLE OFFERS LOGIC <<<

    # 1. Aggregate unique Offer IDs into a single string, ensuring types are strings and ignoring NaNs
    # df_eligible_summary = (
    #     df_universe.groupby('LID')['Offer ID']
    #     .apply(lambda x: ', '.join(set(x.dropna().astype(str))))
    #     .reset_index()
    # )

    # # 2. Rename the column to your target name
    # df_eligible_summary.rename(columns={'Offer ID': 'eligible_offer_ids'}, inplace=True)

    # # 3. Merge using left_on and right_on since the column names differ
    # df_1_1 = df_1_1.merge(
    #     df_eligible_summary, 
    #     left_on='listing_id', 
    #     right_on='LID', 
    ...
    # # 4. Fill missing values for listings that had no eligible offers
    # df_1_1['eligible_offer_ids'] = df_1_1['eligible_offer_ids'].fillna('None')

    # # Optional: Drop the redundant 'LID' column created by the merge
    # if 'LID' in df_1_1.columns:
    #     df_1_1.drop(columns=['LID'], inplace=True)

    # >>> END: NEW ELIGIBLE OFFERS LOGIC <<<
...
<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd;">
    <thead>
        ...
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Order DRR (Input Set)</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Input Coverage %</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Opted in Order DRR (Input Set)</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkgreen; color: white;">DW Opted-in (Of Overall) %</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkgreen; color: white;">DW Opted-in (Of Input) %</th>
        </tr>
    </thead>
    <tbody>
'''
    ...
    for index, row in unique_listing_ids_df_summary_owner_2.iterrows():
        ...
        html_table_owner += f'''
            ...
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['orders_overall']:.0f}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['orders_input']:.0f}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{(row['orders_input']/row['orders_overall'] * 100 if row['orders_overall'] > 0 else 0):.0f}%</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['opted_in_orders_input']:.0f}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{(row['opted_in_orders_input']/row['orders_overall'] * 100 if row['orders_overall'] > 0 else 0):.0f}%</td>
    ...
    #------------------NEW--CUT-FOR--NEWBU----FROM--STARTS--HERE-------------CODE EDITED HERE-----------------
    #----------New BU part

     # Input SET Summary
    input_df_BU = df_1_edited[df_1_edited['Set'] == 'INPUT SET']
    summary_input_BU = input_df_BU.groupby('new_bu').agg(
    orders=('orders', 'sum'),
    opted_in_orders=('orders', lambda x: (x[input_df_BU.loc[x.index, 'Opted-in listing_id'] == 'Opted-in']).sum())
    ).reset_index()

    #summary_input_BU.rename(columns={'New_BU': 'KAM'}, inplace=True)
    summary_input_BU.columns = [col if col == 'new_bu' else col + '_input' for col in summary_input_BU.columns]

    # Non-Input SET Summary
    non_input_df_BU = df_1_edited[df_1_edited['Set'] == 'NON-INPUT SET']
    summary_non_input_BU = non_input_df_BU.groupby('new_bu').agg(
    orders=('orders', 'sum'),
    opted_in_orders=('orders', lambda x: (x[non_input_df_BU.loc[x.index, 'Opted-in listing_id'] == 'Opted-in']).sum())
    ).reset_index()

    #summary_non_input_BU.rename(columns={'New_BU': 'KAM'}, inplace=True)
    summary_non_input_BU.columns = [col if col == 'new_bu' else col + '_non_input' for col in summary_non_input_BU.columns]

    # Overall Summary
    summary_overall_BU = df_1_edited.groupby('new_bu').agg(
    orders=('orders', 'sum'),
    opted_in_orders=('orders', lambda x: (x[df_1_edited.loc[x.index, 'Opted-in listing_id'] == 'Opted-in']).sum())
    ).reset_index()

    #summary_overall_BU.rename(columns={'New_BU': 'KAM'}, inplace=True)
    summary_overall_BU.columns = [col if col == 'new_bu' else col + '_overall' for col in summary_overall_BU.columns]

    # Final merge
    summary_final_BU = summary_input_BU.merge(summary_non_input_BU, on='new_bu', how='outer')
    summary_final_BU = summary_final_BU.merge(summary_overall_BU, on='new_bu', how='outer')

    unique_listing_ids_df_summary_BU = summary_final_BU


    import numpy as np
    # unique_listing_ids_df_summary_BU['Opt In LID %']=np.where(unique_listing_ids_df_summary_BU['listing_id']==0, 0, unique_listing_ids_df_summary_BU['opted_in_listing_id']/unique_listing_ids_df_summary_BU['listing_id']).round(2)
    # unique_listing_ids_df_summary_BU['Opt In SID %']=np.where(unique_listing_ids_df_summary_BU['seller_id']==0, 0, unique_listing_ids_df_summary_BU['opted_in_seller_id']/unique_listing_ids_df_summary_BU['seller_id']).round(2)
    unique_listing_ids_df_summary_BU['DW Input %']=np.where(unique_listing_ids_df_summary_BU['orders_input']==0, 0, unique_listing_ids_df_summary_BU['opted_in_orders_input']/unique_listing_ids_df_summary_BU['orders_input']).round(2)

    # unique_listing_ids_df_summary_BU['Opt In LID %'] = (unique_listing_ids_df_summary_BU['Opt In LID %'] * 100).round(2).astype(str) + '%'
    # unique_listing_ids_df_summary_BU['Opt In SID %'] = (unique_listing_ids_df_summary_BU['Opt In SID %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU['DW Input %'] = (unique_listing_ids_df_summary_BU['DW Input %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU
   
   

    unique_listing_ids_df_summary_BU['DW Non Input %']=np.where(unique_listing_ids_df_summary_BU['orders_non_input']==0, 0, unique_listing_ids_df_summary_BU['opted_in_orders_non_input']/unique_listing_ids_df_summary_BU['orders_non_input']).round(2)
   
    # unique_listing_ids_df_summary_BU['Opt In LID %'] = (unique_listing_ids_df_summary_BU['Opt In LID %'] * 100).round(2).astype(str) + '%'
    # unique_listing_ids_df_summary_BU['Opt In SID %'] = (unique_listing_ids_df_summary_BU['Opt In SID %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU['DW Non Input %'] = (unique_listing_ids_df_summary_BU['DW Non Input %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU

   
    unique_listing_ids_df_summary_BU['DW Overall %']=np.where(unique_listing_ids_df_summary_BU['orders_overall']==0, 0, unique_listing_ids_df_summary_BU['opted_in_orders_overall']/unique_listing_ids_df_summary_BU['orders_overall']).round(2)
   
    # unique_listing_ids_df_summary_BU['Opt In LID %'] = (unique_listing_ids_df_summary_BU['Opt In LID %'] * 100).round(2).astype(str) + '%'
    # unique_listing_ids_df_summary_BU['Opt In SID %'] = (unique_listing_ids_df_summary_BU['Opt In SID %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU['DW Overall %'] = (unique_listing_ids_df_summary_BU['DW Overall %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU
 

    total_row_BU = pd.Series({
        'new_bu': 'Total',
        # 'listing_id': df_1_edited['LID'].nunique(),
        # 'opted_in_listing_id': df_1_edited[df_1_edited['Opted-in listing_id'] == 'Opted-in']['LID'].nunique(),
        # 'seller_id': df_1_edited['Seller Id'].nunique(),
        # 'opted_in_seller_id': df_1_edited[df_1_edited['Opted-in seller_id'] == 'Opted-in']['Seller Id'].nunique(),
        'orders_input': input_df_BU['orders'].sum(),
        'opted_in_orders_input': input_df_BU[input_df_BU['Opted-in listing_id'] == 'Opted-in']['orders'].sum(),
       
        'orders_non_input': non_input_df_BU['orders'].sum(),
        'opted_in_orders_non_input': non_input_df_BU[non_input_df_BU['Opted-in listing_id'] == 'Opted-in']['orders'].sum(),
       
        'orders_overall': df_1_edited['orders'].sum(),
        'opted_in_orders_overall': df_1_edited[df_1_edited['Opted-in listing_id'] == 'Opted-in']['orders'].sum()
    })

    total_row_BU_df = pd.DataFrame([total_row_BU])

    total_row_BU_df['DW Input %'] = np.where(
        total_row_BU_df['orders_input'] == 0, 0,
        total_row_BU_df['opted_in_orders_input'] / total_row_BU_df['orders_input']
    ).round(2)

    total_row_BU_df['DW Non Input %'] = np.where(
        total_row_BU_df['orders_non_input'] == 0, 0,
        total_row_BU_df['opted_in_orders_non_input'] / total_row_BU_df['orders_non_input']
    ).round(2)

    total_row_BU_df['DW Overall %'] = np.where(
        total_row_BU_df['orders_overall'] == 0, 0,
        total_row_BU_df['opted_in_orders_overall'] / total_row_BU_df['orders_overall']
    ).round(2)

    for col in ['DW Input %', 'DW Non Input %', 'DW Overall %']:
        total_row_BU_df[col] = (total_row_BU_df[col] * 100).round(2).astype(str) + '%'

    unique_listing_ids_df_summary_BU_2 = pd.concat([total_row_BU_df, unique_listing_ids_df_summary_BU], ignore_index=True)
    unique_listing_ids_df_summary_BU_2

    unique_listing_ids_df_summary_BU_2=unique_listing_ids_df_summary_BU_2.fillna(0)

    # Build the HTML table with grid lines
    #html_table_BU = '<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd; background-color: yellow;">'
    # html_table_BU = '<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd;">'
    # html_table_BU += '<thead><tr><th style="border: 1px solid #ddd; padding: 8px;">New_BU</th><th style="border: 1px solid #ddd; padding: 8px;">orders_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">opted_in_orders_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">orders_non_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">opted_in_orders_non_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">orders_overall_drr</th><th style="border: 1px solid #ddd; padding: 8px;">opted_in_orders_overall_drr</th><th style="border: 1px solid #ddd; padding: 8px;">DW Input %</th><th style="border: 1px solid #ddd; padding: 8px;">DW Non Input %</th><th style="border: 1px solid #ddd; padding: 8px;">DW Overall %</th></tr></thead>'
    # html_table_BU += '<tbody>'
    html_table_BU = '''
<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd;">
    <thead>
        <tr>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkred; color: white;">new_bu</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Orders DRR (Overall)</th> 
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Order DRR (Input Set)</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Input Coverage %</th>
        <th style="border: 1px solid #ddd; padding: 8px; background-color: darkblue; color: white;">Opted in Order DRR (Input Set)</th>
...
'''
    ...
    # Loop through the dataframe to add rows with conditional formatting
    for index, row in unique_listing_ids_df_summary_BU_2.iterrows():
        if 'total' in str(row['new_bu']).lower():
            row_style = 'background-color: lightskyblue; font-weight: bold;'
        else:
            row_style = ''  # no special style
        html_table_BU += f'''
            <tr style="{row_style}">
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['new_bu']}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['orders_overall']:.0f}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['orders_input']:.0f}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{(row['orders_input']/row['orders_overall'] * 100):.0f}%</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['opted_in_orders_input']:.0f}</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{(row['opted_in_orders_input']/row['orders_overall'] * 100):.0f}%</td>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: left;">{row['DW Input %']}</td>                                                                   
            </tr>
        '''
    ...
    # Combine the CSS and HTML table
    html_table_BU = css_style_BU + html_table_BU




#------------------NEW-CUT-FOR-NEWBU------------------ENDS HERE
...
#-----------------KAM--LEVEL-BU-------COde--starts--here-----------------------------------------------

    input_df_BU_kam = df_1_edited_kam[df_1_edited_kam['Set'] == 'INPUT SET']
    summary_input_BU_kam = input_df_BU_kam.groupby('new_bu').agg(
    orders=('orders', 'sum'),
    opted_in_orders=('orders', lambda x: (x[input_df_BU_kam.loc[x.index, 'Opted-in listing_id'] == 'Opted-in']).sum())
    ).reset_index()

    #summary_input_BU_kam.rename(columns={'new_bu': 'KAM'}, inplace=True)
    summary_input_BU_kam.columns = [col if col == 'new_bu' else col + '_input' for col in summary_input_BU_kam.columns]

    # Non-Input SET Summary
    non_input_df_BU_kam = df_1_edited_kam[df_1_edited_kam['Set'] == 'NON-INPUT SET']
    summary_non_input_BU_kam = non_input_df_BU_kam.groupby('new_bu').agg(
    orders=('orders', 'sum'),
    opted_in_orders=('orders', lambda x: (x[non_input_df_BU_kam.loc[x.index, 'Opted-in listing_id'] == 'Opted-in']).sum())
    ).reset_index()

    #summary_non_input_BU_kam.rename(columns={'new_bu': 'KAM'}, inplace=True)
    summary_non_input_BU_kam.columns = [col if col == 'new_bu' else col + '_non_input' for col in summary_non_input_BU_kam.columns]

    # Overall Summary
    summary_overall_BU_kam = df_1_edited_kam.groupby('new_bu').agg(
    orders=('orders', 'sum'),
    opted_in_orders=('orders', lambda x: (x[df_1_edited_kam.loc[x.index, 'Opted-in listing_id'] == 'Opted-in']).sum())
    ).reset_index()

    #summary_overall_BU_kam.rename(columns={'new_bu': 'KAM'}, inplace=True)
    summary_overall_BU_kam.columns = [col if col == 'new_bu' else col + '_overall' for col in summary_overall_BU_kam.columns]

    # Final merge
    summary_final_BU_kam = summary_input_BU_kam.merge(summary_non_input_BU_kam, on='new_bu', how='outer')
    summary_final_BU_kam = summary_final_BU_kam.merge(summary_overall_BU_kam, on='new_bu', how='outer')

    unique_listing_ids_df_summary_BU_kam = summary_final_BU_kam


    import numpy as np
    # unique_listing_ids_df_summary_BU_kam['Opt In LID %']=np.where(unique_listing_ids_df_summary_BU_kam['listing_id']==0, 0, unique_listing_ids_df_summary_BU_kam['opted_in_listing_id']/unique_listing_ids_df_summary_BU_kam['listing_id']).round(2)
    # unique_listing_ids_df_summary_BU_kam['Opt In SID %']=np.where(unique_listing_ids_df_summary_BU_kam['seller_id']==0, 0, unique_listing_ids_df_summary_BU_kam['opted_in_seller_id']/unique_listing_ids_df_summary_BU_kam['seller_id']).round(2)
    unique_listing_ids_df_summary_BU_kam['DW Input %']=np.where(unique_listing_ids_df_summary_BU_kam['orders_input']==0, 0, unique_listing_ids_df_summary_BU_kam['opted_in_orders_input']/unique_listing_ids_df_summary_BU_kam['orders_input']).round(2)

    # unique_listing_ids_df_summary_BU_kam['Opt In LID %'] = (unique_listing_ids_df_summary_BU_kam['Opt In LID %'] * 100).round(2).astype(str) + '%'
    # unique_listing_ids_df_summary_BU_kam['Opt In SID %'] = (unique_listing_ids_df_summary_BU_kam['Opt In SID %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU_kam['DW Input %'] = (unique_listing_ids_df_summary_BU_kam['DW Input %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU_kam
   
   

    unique_listing_ids_df_summary_BU_kam['DW Non Input %']=np.where(unique_listing_ids_df_summary_BU_kam['orders_non_input']==0, 0, unique_listing_ids_df_summary_BU_kam['opted_in_orders_non_input']/unique_listing_ids_df_summary_BU_kam['orders_non_input']).round(2)
   
    # unique_listing_ids_df_summary_BU_kam['Opt In LID %'] = (unique_listing_ids_df_summary_BU_kam['Opt In LID %'] * 100).round(2).astype(str) + '%'
    # unique_listing_ids_df_summary_BU_kam['Opt In SID %'] = (unique_listing_ids_df_summary_BU_kam['Opt In SID %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU_kam['DW Non Input %'] = (unique_listing_ids_df_summary_BU_kam['DW Non Input %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU_kam

   
    unique_listing_ids_df_summary_BU_kam['DW Overall %']=np.where(unique_listing_ids_df_summary_BU_kam['orders_overall']==0, 0, unique_listing_ids_df_summary_BU_kam['opted_in_orders_overall']/unique_listing_ids_df_summary_BU_kam['orders_overall']).round(2)
   
    # unique_listing_ids_df_summary_BU_kam['Opt In LID %'] = (unique_listing_ids_df_summary_BU_kam['Opt In LID %'] * 100).round(2).astype(str) + '%'
    # unique_listing_ids_df_summary_BU_kam['Opt In SID %'] = (unique_listing_ids_df_summary_BU_kam['Opt In SID %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU_kam['DW Overall %'] = (unique_listing_ids_df_summary_BU_kam['DW Overall %'] * 100).round(2).astype(str) + '%'
    unique_listing_ids_df_summary_BU_kam
 

    total_row_BU_kam = pd.Series({
        'new_bu': 'Total',
        # 'listing_id': df_1_edited_kam['LID'].nunique(),
        # 'opted_in_listing_id': df_1_edited_kam[df_1_edited_kam['Opted-in listing_id'] == 'Opted-in']['LID'].nunique(),
        # 'seller_id': df_1_edited_kam['Seller Id'].nunique(),
        # 'opted_in_seller_id': df_1_edited_kam[df_1_edited_kam['Opted-in seller_id'] == 'Opted-in']['Seller Id'].nunique(),
        'orders_input': input_df_BU_kam['orders'].sum(),
        'opted_in_orders_input': input_df_BU_kam[input_df_BU_kam['Opted-in listing_id'] == 'Opted-in']['orders'].sum(),
       
        'orders_non_input': non_input_df_BU_kam['orders'].sum(),
        'opted_in_orders_non_input': non_input_df_BU_kam[non_input_df_BU_kam['Opted-in listing_id'] == 'Opted-in']['orders'].sum(),
       
        'orders_overall': df_1_edited_kam['orders'].sum(),
        'opted_in_orders_overall': df_1_edited_kam[df_1_edited_kam['Opted-in listing_id'] == 'Opted-in']['orders'].sum()
    })

    total_row_BU_kam_df = pd.DataFrame([total_row_BU_kam])

    total_row_BU_kam_df['DW Input %'] = np.where(
        total_row_BU_kam_df['orders_input'] == 0, 0,
        total_row_BU_kam_df['opted_in_orders_input'] / total_row_BU_kam_df['orders_input']
    ).round(2)

    total_row_BU_kam_df['DW Non Input %'] = np.where(
        total_row_BU_kam_df['orders_non_input'] == 0, 0,
        total_row_BU_kam_df['opted_in_orders_non_input'] / total_row_BU_kam_df['orders_non_input']
    ).round(2)

    total_row_BU_kam_df['DW Overall %'] = np.where(
        total_row_BU_kam_df['orders_overall'] == 0, 0,
        total_row_BU_kam_df['opted_in_orders_overall'] / total_row_BU_kam_df['orders_overall']
    ).round(2)

    for col in ['DW Input %', 'DW Non Input %', 'DW Overall %']:
        total_row_BU_kam_df[col] = (total_row_BU_kam_df[col] * 100).round(2).astype(str) + '%'

    unique_listing_ids_df_summary_BU_kam_2 = pd.concat([total_row_BU_kam_df, unique_listing_ids_df_summary_BU_kam], ignore_index=True)
    unique_listing_ids_df_summary_BU_kam_2

    unique_listing_ids_df_summary_BU_kam_2=unique_listing_ids_df_summary_BU_kam_2.fillna(0)

    # Build the HTML table with grid lines
    #html_table_BU_kam = '<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd; background-color: yellow;">'
    # html_table_BU_kam = '<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd;">'
    # html_table_BU_kam += '<thead><tr><th style="border: 1px solid #ddd; padding: 8px;">new_bu</th><th style="border: 1px solid #ddd; padding: 8px;">orders_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">opted_in_orders_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">orders_non_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">opted_in_orders_non_input_drr</th><th style="border: 1px solid #ddd; padding: 8px;">orders_overall_drr</th><th style="border: 1px solid #ddd; padding: 8px;">opted_in_orders_overall_drr</th><th style="border: 1px solid #ddd; padding: 8px;">DW Input %</th><th style="border: 1px solid #ddd; padding: 8px;">DW Non Input %</th><th style="border: 1px solid #ddd; padding: 8px;">DW Overall %</th></tr></thead>'
    # html_table_BU_kam += '<tbody>'
    html_table_BU_kam = '''
<table class="left-aligned-table" style="width:100%; border-collapse: collapse; border: 1px solid #ddd;">
    <thead>
        <tr>
...
'''
    ...
    # Loop through the dataframe to add rows with conditional formatting
    for index, row in unique_listing_ids_df_summary_BU_kam_2.iterrows():
        if 'total' in str(row['new_bu']).lower():
            row_style = 'background-color: lightskyblue; font-weight: bold;'
        else:
            row_style = ''  # no special style
    ...
    # Combine the CSS and HTML table
    html_table_BU_kam = css_style_BU_kam + html_table_BU_kam

#----------------------------KAM---LEVEL----BU---CUTS--ENDS---HERE-----------------------------------------
    ...
    original_to = ['thotat.vc@flipkart.com','h.kishandasmenon@flipkart.com','vishnuac.vc@flipkart.com','shopsy_buy@flipkart.com', 'shopsy_buy_cr@flipkart.com', 'shopsysell@flipkart.com', 'shopsypricing@flipkart.com','nakul.gulati@flipkart.com','sriram.m@flipkart.com']
    ...
    original_message_id ='<CACZpuczp8cMXSBBGnMC0x3E=VvXujrAcXBSUv5QLhT3jfnVQ9g@mail.gmail.com>'
    ...
    msg['Subject'] = f"New Consolidated Optin Thread - Feb 2026 [L45 Days Txn Set]"
    msg['In-Reply-To'] = original_message_id
    msg['References'] = original_message_id

    body = f"""
    Hi Team, <br><br>
    We are now tracking consolidated optins on the new base file for FEB 2026. The order DRR is calculated from Dec 1st + 50 Days.<br><br>
...
if __name__ == "__main__":
    ...
    folder_id = '1DSKaE5prHsmmUnjNyg4qSmZuAuXFRg78'  # Replace with actual folder ID
    file_path = f"/home/thotat.vc/Downloads/Consolidated_Report_{today}.csv"
    file_path_kam=f"/home/thotat.vc/Downloads/Consolidated_Report_{today}_KAM.csv"
    file_path_re=f"/home/thotat.vc/Downloads/Consolidated_Report_{today}_RE.csv"
    file_path_um=f"/home/thotat.vc/Downloads/Consolidated_Report_{today}_UM.csv" 
