...
SELECT * FROM (
  WITH seller_first_listing AS (
    SELECT
      curr_listing_fact.listing_created_on,
      MONTH(curr_listing_fact.listing_created_on) AS listing_created_month,
      YEAR(curr_listing_fact.listing_created_on) AS listing_created_year,
      --WEEKOFYEAR(curr_listing_fact.listing_created_on) AS listing_created_week,
      s.seller_id,
      s.status,
      s.display_name,
      s.business_name,
  ...
  SELECT
    MONTH(listing_created_on) AS listing_month, 
    --WEEKOFYEAR(listing_created_on) AS listing_week,
  ...
  WHERE YEAR(listing_created_on) = 2026
    AND MONTH(listing_created_on) = ({current_month})
    AND rn = 1
  GROUP BY 
    MONTH(listing_created_on)
    --WEEKOFYEAR(listing_created_on)
) qaas_injected_alias
'''
...
df_listing.to_csv(f"/home/thotat.vc/Downloads/Monthly_onboarded_sellers_{today}.csv",index=False)


filename = f"Monthly_onboarded_sellers_{today}.csv"
file_path = f"/home/thotat.vc/Downloads/Monthly_onboarded_sellers_{today}.csv"
...
# --- RANGE OVERWRITE LOGIC (A5:I8) ---

# 1. Slice the dataframe: 
# We need 4 rows total (A5, A6, A7, A8). 
# We need 9 columns total (A=1 to I=9).
sliced_df = df_listing.iloc[0:4, 0:9]

# 2. Clear the specific range A5:I8 
# This ensures old data in this specific box is wiped before the new data arrives.
cell_list = worksheet.range('A5:I8')
for cell in cell_list:
    cell.value = ''
worksheet.update_cells(cell_list)

# 3. Upload the sliced data starting at A5 (row=5, col=1)
set_with_dataframe(worksheet, sliced_df, row=5, col=1, include_column_header=True)

print("The range A5:I8 has been overwritten successfully.")
